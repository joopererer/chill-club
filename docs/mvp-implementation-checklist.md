...
- [ ] 设计活动推荐权重函数
- [ ] 默认只影响无搜索、无筛选状态下的活动发现页
- [ ] 同一天内排序稳定，不使用每次请求都变化的随机数
- [ ] 明确权重顺序：进行中 / 未开始优先，已结束最低
- [ ] 开始时间接近的活动适度靠前
- [ ] 长时间进行中的活动不要永久占据第一屏
- [ ] 同权重活动使用日期 seed 做稳定洗牌
- [ ] 搜索或筛选后回到明确排序，不使用推荐洗牌
- [ ] 分页结果稳定，刷新后同一页内容不乱跳
- [ ] 不依赖外部 API，不增加数据库字段作为第一版前提
...
- [ ] 桌面端复制按钮视觉层级低于主要报名按钮
- [ ] 新增活动宣传图生成入口
- [ ] 宣传图展示活动标题、时间、地点、费用、报名人数和简短描述
- [ ] 宣传图描述限制字数，避免长文本破坏版式
- [ ] 宣传图包含二维码，扫码跳转当前活动详情页
- [ ] 支持网页端下载宣传图
- [ ] 支持移动端下载或长按保存宣传图
- [ ] 生成图中避免展示隐私信息，例如完整报名用户列表
- [ ] 三语文案同步
